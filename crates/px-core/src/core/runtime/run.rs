...
use super::script::{detect_inline_script, load_inline_python_script, run_inline_script};
...
fn run_reference_target(
    ...
    let commit = match reference.git_ref.as_deref().and_then(normalize_pinned_commit) {
        Some(commit) => commit,
        None => {
    ...
    let provenance_log = record_run_reference_provenance(ctx, &header, &oid, &reference.script_path);
