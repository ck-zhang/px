...
pub(crate) fn refresh_project_site(
    ...
    fn lock_dependency_versions(lock: &px_domain::LockSnapshot) -> HashMap<String, String> {
    ...
    for dep in lock.resolved.iter_mut() {
        ...
        let Some(version) = cache_versions
            .get(&dependency_name(&dep.name).to_ascii_lowercase())
        else {
            continue;
        };
        let dest =
    ...
    if !cached_path_updates.is_empty() {
        ...
        if let Some(tables) = doc.get_mut("dependencies").and_then(Item::as_array_of_tables_mut) {
            for table in tables.iter_mut() {
                ...
                let Some(artifact) = table.get_mut("artifact").and_then(Item::as_table_mut)
                else {
                    continue;
                };
...
fn apply_system_lib_compatibility(
...
mod tests {
    ...
    fn system_lib_compat_caps_unpinned_gdal() -> Result<()> {
        ...
        system_deps.apt_versions.insert(
            "libgdal-dev".to_string(),
            "3.6.2+dfsg-1+b2".to_string(),
        );
    ...
    fn system_lib_compat_preserves_lower_bounds() -> Result<()> {
        let mut system_deps = crate::core::system_deps::SystemDeps::default();
        system_deps.apt_versions.insert("libgdal-dev".to_string(), "3.6.2".to_string());
