...
pub fn python_list(_ctx: &CommandContext, _request: PythonListRequest) -> Result<ExecutionOutcome> {
...
pub fn python_use(ctx: &CommandContext, request: PythonUseRequest) -> Result<ExecutionOutcome> {
    ...
    let Some(record) = runtimes.iter().find(|rt| rt.version == normalized) else {
        return Err(InstallUserError::new(
            format!("runtime {} is not installed", normalized),
            json!({
                "version": normalized,
...
pub fn python_info(_ctx: &CommandContext, _request: PythonInfoRequest) -> Result<ExecutionOutcome> {
    let runtimes = runtime::list_runtimes()?;
