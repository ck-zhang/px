# px Documentation

This docs set replaces the monolithic `spec.md` with focused guides and references. Start here, then dive into the section you need.

Platform support: px targets Linux and macOS today; Windows is not supported yet (use WSL in the meantime).

## Guides

* [Getting started](./guides/getting-started.md)
* [Workspaces](./guides/workspaces.md)
* [Tools](./guides/tools.md)
* [Runtimes](./guides/runtimes.md)

## Reference

* [Concepts overview](./reference/concepts-overview.md)
* [Model](./reference/model.md)
* [State machines](./reference/state-machines.md)
* [Commands](./reference/commands.md)
* [Tools and runtimes API](./reference/tools-and-runtimes-api.md)
* [Errors and troubleshooting](./reference/errors-and-troubleshooting.md)

## Design

* [Determinism and CI](./design/determinism-and-ci.md)
* [Non-goals](./design/non-goals.md)

## Legacy

* [Original spec](./spec-legacy.md) – preserved for history; the modular docs above are the current source of truth.
